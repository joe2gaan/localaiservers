...
_gfx906_persistent_ar_tp_graph_capture_count = 0
...
_gfx906_public_comm_trace_counts: dict[tuple[str, str, str, int, int, bool], int] = {}
_gfx906_public_comm_trace_events = 0


def _gfx906_public_comm_trace_enabled() -> bool:
    return os.getenv("VLLM_GFX906_PUBLIC_COMM_TRACE", "0").lower() in (
        "1",
        "true",
        "yes",
        "on",
...
def _gfx906_public_comm_trace(
    kind: str,
    tensor: torch.Tensor,
    world_size: int,
    extra: str = "",
) -> None:
    if not _gfx906_public_comm_trace_enabled():
        return

    global _gfx906_public_comm_trace_events
    ...
    key = (
        ...
        int(tensor.numel()),
        int(world_size),
        os.getenv("VLLM_GFX906_PERSISTENT_AR_IN_GRAPH_CAPTURE", "0").lower()
        in ("1", "true", "yes", "on"),
    )
    _gfx906_public_comm_trace_counts[key] = (
        _gfx906_public_comm_trace_counts.get(key, 0) + 1
    )
    _gfx906_public_comm_trace_events += 1

    limit = int(os.getenv("VLLM_GFX906_PUBLIC_COMM_TRACE_EVENT_LIMIT", "64"))
    if _gfx906_public_comm_trace_events <= limit:
        print(
            "[gfx906_public_comm_trace "
            f"pid={os.getpid()} event={_gfx906_public_comm_trace_events}] "
            f"kind={kind} shape={shape} dtype={tensor.dtype} "
            f"numel={int(tensor.numel())} world={int(world_size)} "
            f"capture={key[-1]} device={tensor.device} contiguous={tensor.is_contiguous()} "
            f"{extra}",
            file=sys.stderr,
            flush=True,
        )
...
def _gfx906_public_comm_trace_summary() -> None:
    if not _gfx906_public_comm_trace_counts:
        return
    limit = int(os.getenv("VLLM_GFX906_PUBLIC_COMM_TRACE_SUMMARY_LIMIT", "80"))
    items = sorted(
        _gfx906_public_comm_trace_counts.items(),
        key=lambda item: item[1],
        reverse=True,
    )
    print(
        f"[gfx906_public_comm_trace pid={os.getpid()} summary] "
        f"unique={len(items)} events={sum(count for _, count in items)}",
        file=sys.stderr,
        flush=True,
    )
    for idx, (key, count) in enumerate(items[:limit], start=1):
        kind, shape, dtype, numel, world_size, capture = key
        print(
            "[gfx906_public_comm_trace "
            f"pid={os.getpid()} summary#{idx}] count={count} "
            f"kind={kind} shape={shape} dtype={dtype} "
            f"numel={numel} world={world_size} capture={capture}",
            file=sys.stderr,
            flush=True,
        )


atexit.register(_gfx906_public_comm_trace_summary)
...
def all_reduce_inplace(tensor: torch.Tensor, group_name: str) -> None:
    ...
    if group is None:
        raise ValueError(f"Group {group_name} is destroyed.")
    group._all_reduce_inplace(tensor)


def all_reduce_inplace_public(tensor: torch.Tensor, group_name: str) -> None:
    assert group_name in _groups, f"Group {group_name} is not found."
    group = _groups[group_name]()
    if group is None:
        raise ValueError(f"Group {group_name} is destroyed.")
    group._all_reduce_inplace_public(tensor)


def all_reduce_inplace_kind(tensor: torch.Tensor, group_name: str, kind: str) -> None:
    assert group_name in _groups, f"Group {group_name} is not found."
    group = _groups[group_name]()
    if group is None:
        raise ValueError(f"Group {group_name} is destroyed.")
    group._all_reduce_inplace_kind(tensor, kind)


def all_reduce_add_rms_f16(
    tensor: torch.Tensor,
    recv: torch.Tensor,
...
def all_reduce_inplace_kind_fake(
    tensor: torch.Tensor, group_name: str, kind: str
) -> None:
    return None


def all_reduce_add_rms_f16_fake(
    tensor: torch.Tensor,
    recv: torch.Tensor,
    residual_in: torch.Tensor,
    gamma: torch.Tensor,
...
direct_register_custom_op(
    op_name="all_reduce_inplace_public",
    op_func=all_reduce_inplace_public,
    mutates_args=["tensor"],
    fake_impl=all_reduce_inplace_fake,
)

direct_register_custom_op(
    op_name="all_reduce_inplace_kind",
    op_func=all_reduce_inplace_kind,
    mutates_args=["tensor"],
    fake_impl=all_reduce_inplace_kind_fake,
)

direct_register_custom_op(
    op_name="all_reduce_add_rms_f16",
    op_func=all_reduce_add_rms_f16,
    mutates_args=[
        "recv",
        "norm_out",
        "residual_out",
    ...
    fake_impl=all_reduce_add_rms_f16_fake,
)
...
class GroupCoordinator:
    ...
    def graph_capture(self, graph_capture_context: GraphCaptureContext | None = None):
        global _gfx906_persistent_ar_tp_graph_capture_count
        ...
        persistent_ar_env_restore = None
        persistent_ar_capture_env_restore = os.environ.get(
            "VLLM_GFX906_PERSISTENT_AR_IN_GRAPH_CAPTURE"
        )
        os.environ["VLLM_GFX906_PERSISTENT_AR_IN_GRAPH_CAPTURE"] = "1"
        persistent_ar_skip_capture = False
        if self.unique_name.startswith("tp:") and os.getenv(
            "VLLM_GFX906_PERSISTENT_AR", "0"
        ).lower() in ("1", "true", "yes"):
            _gfx906_persistent_ar_tp_graph_capture_count += 1
            skip_contexts = int(
                os.getenv("VLLM_GFX906_PERSISTENT_AR_SKIP_GRAPH_CAPTURE_CONTEXTS", "0")
            )
            persistent_ar_skip_capture = (
                _gfx906_persistent_ar_tp_graph_capture_count <= skip_contexts
            )
            if persistent_ar_skip_capture:
                persistent_ar_env_restore = os.environ.get("VLLM_GFX906_PERSISTENT_AR")
                os.environ["VLLM_GFX906_PERSISTENT_AR"] = "0"

        if (not persistent_ar_skip_capture) and os.getenv(
            "VLLM_GFX906_PERSISTENT_AR_PREINIT_ON_GRAPH_CAPTURE", "0"
        ).lower() in ("1", "true", "yes"):
            pynccl_comm = None
            if self.device_communicator is not None:
            ...
            if pynccl_comm is not None and not getattr(pynccl_comm, "disabled", False):
                from vllm.distributed.device_communicators.gfx906_persistent_ar import (
                    preinit_for_graph_capture,
                )

                preinit_for_graph_capture(pynccl_comm, torch.cuda.current_device())

        try:
            with torch.cuda.stream(stream), maybe_ca_context:
                yield graph_capture_context
        finally:
            if persistent_ar_capture_env_restore is None:
                os.environ.pop("VLLM_GFX906_PERSISTENT_AR_IN_GRAPH_CAPTURE", None)
            else:
                os.environ["VLLM_GFX906_PERSISTENT_AR_IN_GRAPH_CAPTURE"] = (
                    persistent_ar_capture_env_restore
                )
            if persistent_ar_env_restore is not None:
                os.environ["VLLM_GFX906_PERSISTENT_AR"] = persistent_ar_env_restore
    ...
    def all_reduce_inplace(self, input_: torch.Tensor) -> torch.Tensor:
        """
        Mutate ``input_`` with a TP all-reduce and return the same tensor.

        This is only intended for fresh RowParallel outputs on gfx906 source
        experiments. The generic all_reduce path stays out-of-place because
        most callers rely on vLLM's pure custom-op contract.
        """
        if self.world_size == 1:
            return input_

        if self.use_custom_op_call:
            torch.ops.vllm.all_reduce_inplace(input_, group_name=self.unique_name)
            return input_
        self._all_reduce_inplace(input_)
        return input_

    def all_reduce_inplace_kind(self, input_: torch.Tensor, kind: str) -> torch.Tensor:
        """
        In-place TP all-reduce with a static row-parallel family label.

        The kinded custom op lets gfx906 experiments choose different sidecar
        libraries for attention and MLP reductions while preserving the same
        graph-capture contract as the unkinded in-place op.
        """
        if self.world_size == 1:
            return input_
        ...
        if self.use_custom_op_call:
            torch.ops.vllm.all_reduce_inplace_kind(
                input_, group_name=self.unique_name, kind=kind
            )
            return input_
        self._all_reduce_inplace_kind(input_, kind)
        return input_
    ...
    def all_reduce_add_rms_f16(
        ...
        if self.world_size == 1:
            recv.copy_(input_)
            residual_out.copy_(residual_in + recv.float())
            norm_out.copy_(residual_out.to(dtype=norm_out.dtype))
            return norm_out, residual_out

        if self.use_custom_op_call:
            torch.ops.vllm.all_reduce_add_rms_f16(
                input_,
                recv,
                residual_in,
                gamma,
    ...
    def _all_reduce_out_place(self, input_: torch.Tensor) -> torch.Tensor:
        ...
        _gfx906_public_comm_trace(
            "all_reduce_out_place",
            input_,
            self.world_size,
            extra=f"group={self.unique_name}",
        )
        return self.device_communicator.all_reduce(input_)

    def _all_reduce_inplace(self, input_: torch.Tensor) -> torch.Tensor:
        if self.device_communicator is None:
            raise ValueError("No device communicator found")

        pynccl_comm = getattr(self.device_communicator, "pynccl_comm", None)
        if pynccl_comm is not None and not getattr(pynccl_comm, "disabled", False):
            if os.getenv("VLLM_GFX906_PERSISTENT_AR", "0").lower() in (
                "1",
                "true",
                "yes",
            ):
                from vllm.distributed.device_communicators.gfx906_persistent_ar import (
                    try_all_reduce_inplace,
                )

                if try_all_reduce_inplace(pynccl_comm, input_):
                    return input_
                _gfx906_public_comm_trace(
                    "all_reduce_inplace_sidecar_miss",
                    input_,
                    self.world_size,
                    extra=(
                        f"group={self.unique_name} "
                        f"pynccl_disabled={getattr(pynccl_comm, 'disabled', False)}"
                    ),
                )
            out = pynccl_comm.all_reduce(input_, input_)
            if out is not None:
                return input_

        _gfx906_public_comm_trace(
            "all_reduce_inplace_torch_fallback",
            input_,
            self.world_size,
            extra=f"group={self.unique_name}",
        )
    ...
    def _all_reduce_inplace_kind(
        self, input_: torch.Tensor, kind: str
    ) -> torch.Tensor:
        if self.device_communicator is None:
            raise ValueError("No device communicator found")
        ...
        if pynccl_comm is not None and not getattr(pynccl_comm, "disabled", False):
            if os.getenv("VLLM_GFX906_PERSISTENT_AR", "0").lower() in (
                ...
                if try_all_reduce_inplace(pynccl_comm, input_, kind):
                    return input_
                _gfx906_public_comm_trace(
                    "all_reduce_inplace_kind_sidecar_miss",
                    input_,
                    self.world_size,
                    extra=(
                        f"group={self.unique_name} kind={kind} "
                        f"pynccl_disabled={getattr(pynccl_comm, 'disabled', False)}"
                    ),
                )
            out = pynccl_comm.all_reduce(input_, input_)
        ...
        _gfx906_public_comm_trace(
            "all_reduce_inplace_kind_torch_fallback",
            input_,
            self.world_size,
            extra=f"group={self.unique_name} kind={kind}",
        )
        torch.distributed.all_reduce(input_, group=self.device_group)
        return input_

    def _all_reduce_add_rms_f16(
        self,
        input_: torch.Tensor,
        recv: torch.Tensor,
        residual_in: torch.Tensor,
        ...
        if pynccl_comm is not None and not getattr(pynccl_comm, "disabled", False):
            if os.getenv("VLLM_GFX906_PERSISTENT_AR", "0").lower() in (
                ...
                if try_add_rms_f16(
                    ...
                    return norm_out, residual_out
        raise RuntimeError(
            "gfx906 sidecar add_rms_f16 did not run; no fallback is defined "
            "for the experimental boundary rewrite"
        )

    def _all_reduce_inplace_public(self, input_: torch.Tensor) -> torch.Tensor:
        if self.device_communicator is None:
            raise ValueError("No device communicator found")
        ...
        if pynccl_comm is not None and not getattr(pynccl_comm, "disabled", False):
            _gfx906_public_comm_trace(
                "all_reduce_inplace_public_pynccl",
                input_,
                self.world_size,
                extra=f"group={self.unique_name}",
            )
        ...
        _gfx906_public_comm_trace(
            "all_reduce_inplace_public_torch_fallback",
            input_,
            self.world_size,
            extra=f"group={self.unique_name}",
        )
    ...
    def _all_gather_out_place(self, input_: torch.Tensor, dim: int) -> torch.Tensor:
        ...
        _gfx906_public_comm_trace(
            ...
            input_,
            self.world_size,
            extra=f"group={self.unique_name} dim={dim}",
        )
