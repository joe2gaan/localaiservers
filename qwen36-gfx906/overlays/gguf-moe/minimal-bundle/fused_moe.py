...
def _qwen_c1_topk8_w1_act_kernel(
    ...
    expert = tl.load(topk_ids + pid_m * TOP_K + pid_k).to(tl.int64)
    acc_gate = tl.zeros((BLOCK_I,), dtype=tl.float32)
    acc_up = tl.zeros((BLOCK_I,), dtype=tl.float32)
    for h0 in range(0, H, BLOCK_H):
        h = h0 + offs_h
        xv = tl.load(x + pid_m * H + h, mask=h < H, other=0.0)
        wg = tl.load(
            w1
            + expert * W1_STRIDE_E
    ...
    tl.store(
        act + (pid_m * TOP_K + pid_k) * I + offs_i,
        out.to(tl.float16),
        mask=offs_i < I,
    )
...
def _qwen_c1_topk8_w2_reduce_kernel(
    ...
    for kidx in range(0, TOP_K):
        route_offset = pid_m * TOP_K + kidx
        expert = tl.load(topk_ids + route_offset).to(tl.int64)
        route_w = tl.load(topk_weights + route_offset).to(tl.float32)
        expert_acc = tl.zeros((BLOCK_H,), dtype=tl.float32)
        for i0 in range(0, I, BLOCK_I):
            i = i0 + offs_i
            av = tl.load(
                act + route_offset * I + i,
                mask=i < I,
                other=0.0,
            )
            ww = tl.load(
    ...
    tl.store(y + pid_m * H + offs_h, acc.to(tl.float16), mask=offs_h < H)
...
def _try_qwen_c1_topk8_fastpath(
    ...
    inter = w2.size(2)
    logger.info_once(
        f"qwen c1 topk8 MoE fastpath active tokens={tokens} "
        f"hidden={hidden} inter={inter}"
    )
    act = torch.empty((tokens, 8, inter), device=hidden_states.device, dtype=torch.float16)
    out = torch.empty_like(hidden_states)
    _qwen_c1_topk8_w1_act_kernel[(tokens, 8, triton.cdiv(inter, 8))](
        hidden_states,
        w1,
        topk_ids.view(-1),
        act,
    ...
    _qwen_c1_topk8_w2_reduce_kernel[(tokens, triton.cdiv(hidden, 64))](
        w2,
        topk_ids.view(-1),
        topk_weights.view(-1),
        act,
