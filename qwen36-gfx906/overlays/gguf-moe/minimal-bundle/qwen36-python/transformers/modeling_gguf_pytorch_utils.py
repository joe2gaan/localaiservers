...
# Experimental Qwen3.5-MoE GGUF text-only metadata bridge for qwen35moe.
# This is used only in the copied GGUF experiment image path.
if "qwen35moe" not in GGUF_TO_TRANSFORMERS_MAPPING["config"]:
    _qwen35moe_config = dict(GGUF_TO_TRANSFORMERS_MAPPING["config"].get("qwen3_moe", {}))
    _qwen35moe_config.update({
        "expert_feed_forward_length": "moe_intermediate_size",
        "expert_shared_feed_forward_length": "shared_expert_intermediate_size",
        "attention.value_length": "linear_value_head_dim",
        "ssm.conv_kernel": "linear_conv_kernel_dim",
        "ssm.group_count": "linear_num_key_heads",
        "ssm.inner_size": "linear_hidden_size",
        "ssm.state_size": "linear_state_size",
        "ssm.time_step_rank": "linear_dt_rank",
        "full_attention_interval": None,
        "nextn_predict_layers": None,
        "rope.dimension_sections": None,
    })
    GGUF_TO_TRANSFORMERS_MAPPING["config"]["qwen35moe"] = _qwen35moe_config
    GGUF_CONFIG_MAPPING["qwen35moe"] = _qwen35moe_config
    GGUF_CONFIG_DEFAULTS_MAPPING["qwen35moe"] = {
...
class Qwen35MoeTensorProcessor(MiniMaxM2TensorProcessor):
    def process(self, weights, name: str, **kwargs):
        # llama.cpp stores Qwen3.5-MoE RMSNorm-family weights offset by +1.
        # GatedDeltaNet ssm_norm tensors are already in HF layout and must not
        # be shifted.
...
TENSOR_PROCESSORS = {
    ...
    "qwen35moe": Qwen35MoeTensorProcessor,
}
