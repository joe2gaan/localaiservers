...
class GGUFModelLoader(BaseModelLoader):
    ...
    def _get_gguf_weights_map(self, model_config: ModelConfig):
        ...
        gguf_model_type = model_type
        if model_type == "qwen3_5_moe_text":
            gguf_model_type = "qwen35moe"
        is_multimodal = (
        ...
        if model_type == "qwen3_5_moe_text":
            # qwen35moe GGUF stores a text-only Qwen3.5-MoE backbone.
            # The generic gguf-py map covers most names, but dt_bias and
            # individual expert gate/up/down tensors need explicit routing
            # through the vLLM FusedMoE loader.
            for idx in range(text_config.num_hidden_layers):
                ...
                gguf_to_hf_name_map[f"blk.{idx}.ffn_gate_exps.weight"] = (
                    f"model.layers.{idx}.mlp.experts.gate_proj"
                )
                gguf_to_hf_name_map[f"blk.{idx}.ffn_up_exps.weight"] = (
                    f"model.layers.{idx}.mlp.experts.up_proj"
                )
                sideload_params.append(
                    re.compile(
                        f"model\\.layers\\.{idx}"
        ...
        for key, value in gguf.MODEL_ARCH_NAMES.items():
            if value == gguf_model_type:
                arch = key
                break
        if arch is None:
            raise RuntimeError(f"Unknown gguf model_type: {gguf_model_type}")
