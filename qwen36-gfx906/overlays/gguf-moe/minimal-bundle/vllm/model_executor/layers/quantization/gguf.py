...
class GGUFConfig(QuantizationConfig):
    ...
    def get_quant_method(
        ...
        if isinstance(layer, LinearBase):
            if os.environ.get("VLLM_GFX906_GGUF_FORCE_UNQUANT_LINEAR") == "1":
                logger.info_once(
                    "Forcing unquantized Linear method for GGUF layer %s",
                    prefix,
                )
            ...
            if os.environ.get("VLLM_GFX906_GGUF_FORCE_UNQUANT_LINEAR") == "1":
                logger.info_once(
                    "Forcing unquantized Embedding method for GGUF layer %s",
                    prefix,
                )
            ...
            if os.environ.get("VLLM_GFX906_GGUF_FORCE_UNQUANT_MOE") == "1":
                logger.info_once(
                    "Forcing unquantized FusedMoE method for GGUF layer %s",
                    prefix,
                )
            ...
            if is_layer_skipped_gguf(
                ...
                logger.info_once(
                    "Using unquantized FusedMoE method for F16/BF16/F32 GGUF layer %s",
                    prefix,
                )
                return UnquantizedFusedMoEMethod(layer.moe_config)
