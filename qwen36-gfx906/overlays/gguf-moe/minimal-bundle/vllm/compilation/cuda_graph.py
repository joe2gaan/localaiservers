...
_GFX906_REPLAY_TIMING_FILE = os.environ.get("VLLM_GFX906_CUDAGRAPH_REPLAY_TIMING_FILE")
try:
    _GFX906_REPLAY_TIMING_LIMIT = int(os.environ.get("VLLM_GFX906_CUDAGRAPH_REPLAY_TIMING_LIMIT", "20000"))
except ValueError:
    _GFX906_REPLAY_TIMING_LIMIT = 20000
try:
    _GFX906_REPLAY_TIMING_SAMPLE_MOD = max(1, int(os.environ.get("VLLM_GFX906_CUDAGRAPH_REPLAY_TIMING_SAMPLE_MOD", "1")))
except ValueError:
    _GFX906_REPLAY_TIMING_SAMPLE_MOD = 1
_GFX906_REPLAY_TIMING_COUNTER = 0
_GFX906_REPLAY_TIMING_WRITTEN = 0
...
class CUDAGraphWrapper:
    ...
    def __call__(self, *args: Any, **kwargs: Any) -> Any | None:
        ...
        global _GFX906_REPLAY_TIMING_COUNTER, _GFX906_REPLAY_TIMING_WRITTEN
        _GFX906_REPLAY_TIMING_COUNTER += 1
        should_time = (
            _GFX906_REPLAY_TIMING_FILE
            and _GFX906_REPLAY_TIMING_WRITTEN < _GFX906_REPLAY_TIMING_LIMIT
            and (_GFX906_REPLAY_TIMING_COUNTER % _GFX906_REPLAY_TIMING_SAMPLE_MOD == 0)
        )
        if should_time:
            ...
            _GFX906_REPLAY_TIMING_WRITTEN += 1
            try:
                with open(_GFX906_REPLAY_TIMING_FILE, "a", encoding="utf-8") as f:
                    f.write(f"pid={os.getpid()} call={_GFX906_REPLAY_TIMING_COUNTER} mode={self.runtime_mode.name} batch={entry.batch_descriptor} elapsed_ms={elapsed_ms:.6f}\n")
            except Exception:
                pass
        else:
            get_offloader().sync_prev_onload()
