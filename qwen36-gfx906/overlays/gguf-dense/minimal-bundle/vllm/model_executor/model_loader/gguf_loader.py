...
class GGUFModelLoader(BaseModelLoader):
    ...
    def _get_gguf_weights_map(self, model_config: ModelConfig):
        ...
        if config.model_type in ("qwen3_5", "qwen3_5_moe"):
            # The Qwen3.5/Qwen3.6 GGUF artifacts tested here are text-only.
            # Their HF configs can expose a vision_config object, but gguf-py
            # text model mapping should not attempt to build a multimodal
            # projector map for these files.
            is_multimodal = False
        gguf_to_hf_name_map = {}
        ...
        if model_type == "qwen3_5":
            # Qwen3.5/Qwen3.6 dense models use "qwen3_5" in HuggingFace
            # configs while gguf-py names the corresponding architecture
            # "qwen35".
            model_type = "qwen35"
        if model_type == "qwen3_5_moe":
            # Qwen3.5/Qwen3.6 MoE models use "qwen3_5_moe" in HuggingFace
            # configs while gguf-py names the corresponding architecture
            # "qwen35moe".
            model_type = "qwen35moe"
        if model_type in ("qwen35", "qwen35moe"):
            # gguf-py maps the Qwen3.5/Qwen3.6 SSM/linear-attention tensors
            # except dt_bias in this image. Suffixless tensors such as
            # blk.N.ssm_a also need an explicit map because they are real GGUF
            # keys, not ".weight" or ".bias" parameters.
            layer_types = getattr(text_config, "layer_types", None)
            for idx in range(text_config.num_hidden_layers):
                layer_type = layer_types[idx] if layer_types is not None else None
                if layer_type in (None, "linear_attention"):
                ...
                if layer_type == "full_attention":
                    gguf_to_hf_name_map[f"blk.{idx}.attn_gate.weight"] = (
                        f"model.layers.{idx}.self_attn.gate_proj.weight"
                    )
        ...
        dummy_config = text_config if config.model_type in ("qwen3_5", "qwen3_5_moe") else config
        ...
        def find_hf_name_in_tensor_map(hf_name: str) -> str | None:
            ...
            return gguf_name if suffix == "" else gguf_name + "." + suffix
        ...
        if config.model_type == "qwen3_5" and any(
            arch == "Qwen3_5ForConditionalGeneration"
            for arch in (getattr(config, "architectures", None) or [])
        ):
            # The Qwen3.5 conditional wrapper exposes text tensors under
            # language_model.*. The plain CausalLM path keeps model.* / lm_head.*.
            gguf_to_hf_name_map = {
                key: (
                    f"language_model.{value}"
                    if value.startswith(("model.", "lm_head."))
                    else value
                )
                for key, value in gguf_to_hf_name_map.items()
            }
    ...
    def _get_gguf_weight_type(
        ...
        if model_config.hf_config.model_type in ("qwen3_5", "qwen3_5_moe"):
            is_multimodal = False
        if is_multimodal:
    ...
    def _get_weights_iterator(
        ...
        is_multimodal = (
            hasattr(hf_config, "vision_config") and hf_config.vision_config is not None
        )
    ...
    def load_model(
        ...
        with set_default_torch_dtype(model_config.dtype):
            ...
            if getattr(model_config.hf_config, "model_type", None) in (
                "qwen3_5",
                "qwen3_5_moe",
            ):
                try:
                    language_model = getattr(model, "language_model", None)
                    causal_model = language_model if language_model is not None else model
                    core_model = getattr(causal_model, "model", None)
                    ...
                    if layers is not None and len(layers) > 0:
                        ...
                        if la is not None:
                            for mod_name in (
                                ...
                                mod = getattr(la, mod_name, None)
                                if mod is None:
                                    continue
                                for attr in ("weight", "qweight", "qweight_type"):
                                    ...
                                    print(
                                        ...
                                        shape,
                                        "weight_type",
                                        getattr(param, "weight_type", None),
                                        flush=True,
                                    )
                    print(
                        "GGUF_QWEN35_POST_MODULES",
                        sum(1 for _ in model.named_modules()),
                        flush=True,
                    )
                except Exception as exc:
                    print("GGUF_QWEN35_POST_DIAG_ERROR", repr(exc), flush=True)
        return model
