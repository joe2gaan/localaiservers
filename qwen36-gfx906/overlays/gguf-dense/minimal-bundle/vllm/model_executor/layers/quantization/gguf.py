...
class GGUFConfig(QuantizationConfig):
    ...
    def get_quant_method(
        ...
        if isinstance(layer, LinearBase):
            ...
            if (
                os.environ.get("VLLM_GGUF_EMBED_TOKENS_UNQUANT") == "1"
                and prefix.split(".")[-1] == "embed_tokens"
            ):
                logger.warning_once(
                    "VLLM_GGUF_EMBED_TOKENS_UNQUANT forcing %s to "
                    "UnquantizedEmbeddingMethod",
                    prefix,
                )
...
def _fused_mul_mat_gguf(
    ...
    if qweight_type in UNQUANTIZED_TYPES:
        return x @ qweight.T
    if os.environ.get("VLLM_GGUF_FORCE_DEQUANT_MATMUL") == "1":
...
class GGUFLinearMethod(LinearMethodBase):
    ...
    def apply(
        ...
        if shard_id:
            # Qwen3.5 GGUF may yield linear-attention Z before QKV.
            # Apply merged GGUF shards in logical shard-id order so
            # in_proj_qkvz returns [Q, K, V, Z], independent of file order.
...
class GGUFEmbeddingMethod(GGUFLinearMethod):
    ...
    def embedding(self, layer: torch.nn.Module, x: torch.Tensor) -> torch.Tensor:
        qweight = layer.qweight
        if isinstance(qweight, GGUFUninitializedParameter) and len(qweight.data_container) == 1:
            data = qweight.data_container[0]
            padded_param = Parameter(data, requires_grad=False)
            qweight.data_container.clear()
            set_weight_attrs(padded_param, vars(qweight))
            layer.register_parameter("qweight", padded_param)
