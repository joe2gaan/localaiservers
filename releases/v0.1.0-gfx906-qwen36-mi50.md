...
validated separately by the manifest digest above. On hosts where the user is in the
Docker group but does not have noninteractive sudo, set
`DOCKER_ISOLATED_DAEMON_ENABLED=0` for the prebuilt image path. For first-run model
staging, set `HF_HUB_DISABLE_XET=1` to use the standard Hugging Face download path.
With `AUTO_STAGE_MODEL=1`, the deploy script downloads and verifies the model snapshot
