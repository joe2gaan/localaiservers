...
only when the exported Docker archive matches this SHA. Current `deploy.sh` defaults
to `BYTE_FOR_BYTE_VALIDATION_MODE=1`, which fails the source-build path on a mismatch;
use `BYTE_FOR_BYTE_VALIDATION_MODE=0` only for non-canonical local deploys where no
