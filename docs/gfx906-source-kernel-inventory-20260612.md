...
- Dense 27B primary gate remains open. Best current source high-water is the
  graph-safe non-resident sidecar Tree/LL allreduce runtime plus exact
  public-RCCL fallback for captured attention `call_site=1664`, on top of
  balanced `RCCL_TREES`, ROCm 7.2, O3 async, mutable RowParallel,
  native-runtime interleaved SwiGLU, Tree/LL, P2P disabled, four NCCL channels,
  and the `2048/1536/1536` sidecar descriptor split: `.20`
  `c1_128/c1_2000/c1_10000 = 63.084/63.993/60.380` backend TPS. The latest
  `.30` support run for the same exact fallback reached `62.197/63.034/59.525`.
  The current winner extends the non-resident sidecar to graph-capture wider
  rows, but the hot strict single-request decode region still replays mostly
  `1x5120` sidecar work.
- MoE 35B TP4 publication gate is cleared on the C1 topk8 MoE fastpath stack:
  corrected hard-thinking confirmation reached `95.661` backend TPS on
  `c1_10000`.
- The dense gate is now clearly an MLP/down RowParallel collective-boundary
  problem. The current clean-window profile is still NCCL/RCCL dominated:
  NCCL `59.03%`, LLGemm `17.06%`, interleaved SwiGLU `12.54%`.
- The target-selective sidecar selector is graph-safe through a second custom
  op, and a 2026-06-13 source fix corrected attention detection from
  `input_size_per_partition == 640` to `(640, 768)`. The corrected full
  ladders still reject selective routing: `.20` attention-only reached
  `57.793/58.527/55.468`, `.30` `mlp,attn` reached
  `55.509/56.187/53.417`, `.20` `mlp`-only reached
  `57.420/58.130/55.138`, and `.30` `mlp`-only reached
  `55.238/55.824/52.996` backend TPS for strict c1 / c1_2000 / c1_10000.
- Sidecar worker-shape tuning is rejected beyond the current `nWarps=4` /
  256-thread launch: `3/192` is exact but slower, while `4/320`, `5/320`,
  `6/384`, and `8/512` fault at launch on both checked hosts.
- The latest profile of the actual sidecar winner shifts the next serious
  source target to the sidecar collective itself. `.30`
  `qwen36_27b_sidecar_winner_rocprof_h30_20260613_004845` shows
  `sidecar_tree_ll_1x5120_kernel` at `13.767s`, `54.14%`, `98,062` launches,
  and about `140 us` average launch time in the profiled c1 window. A
  same-stack `.20` profile
  `qwen36_27b_sidecar_winner_rocprof_h20_20260613_sidecar_winner_h20_profile`
  reproduces the structure with faster collective rows: sidecar `12.483s`,
  `52.63%`, `98,059` launches, `127 us` average; public
  `ncclDevKernel_Generic_4` remains `2.014s`, `8.49%`, `1,672` launches.
  Follow-up public-communication tracing showed those residual public NCCL
  rows are mostly prefill and CUDA-graph capture shapes (`2048x5120`,
  `2/4/8x5120`), while exact `1x5120` decode allreduces still route through
  the sidecar. Multi-row sidecar support is now a prefill/concurrency side
  milestone, not the main single-request decode gate path. The multi-row
  candidate nevertheless produced a small repeatable full-ladder gain and is
  the current high-water. Follow-up tail analysis on `.20` winner, `.30`
  original winner, and `.30` multi-row winner profiles shows normal sidecar
  launch intervals near `220 us` but local p99 duration tails around
  `3.1-3.6 ms`, with rare longer local spikes. The tail is not cleanly
  rank-synchronized, so simple rank-stagger or external scheduling tweaks are
  no longer the serious path; the next source work needs to reduce hot
  `1x5120` launch count or change the sidecar collective primitive.
- Rechecking the current sidecar high-water with `NCCL_NTHREADS` unset is
  closed as a no-promotion guardrail. `.20` reached
  `62.213/63.195/59.685` and `.30` reached `61.972/62.786/59.137` backend TPS
  for strict c1 / c1_2000 / c1_10000. Keep `NCCL_NTHREADS=128` in the dense
  sidecar serving envelope. Rechecking unset again after the multi-row sidecar
  change was also mixed (`.20` `62.218/63.237/59.704`, `.30`
  `62.031/62.948/59.237`) and did not promote.
- Explicit RCCL override variables are also closed for the current dense
  sidecar envelope. `RCCL_OVERRIDE_ALGO=Tree` plus `RCCL_OVERRIDE_PROTO=LL`
  produced valid strict c1 smokes at `.20` `62.275` and `.30` `62.036`
  backend TPS, so they do not improve over the same-host high-water.
- Host `.10` does not reproduce the `.20/.30` sidecar high-water after overlay
  hashes were corrected: strict/c1_2000/c1_10000 backend TPS were
  `54.843/54.760/51.270`. Direct VRAM-byte checks still show all eight GPUs at
  `34342961152` bytes, so this is treated as a host/topology lane-quality
  rejection rather than a 16 GB-card issue.
- Host `.50` is not a valid ROCm 7.2 dense comparison lane yet. It has a small
  `gfx803` display card plus eight 32 GiB `gfx906` cards. The launcher now
  avoids inherited masks that include physical GPU0, but minimal ROCm 7.2
  diagnostics still fail before model startup with `hsa_init failed with 1000`
  and `hipErrorNoDevice`. ROCm 6.3 initializes a selected MI50 on the same host,
  so this is recorded as a ROCm 7.2 host/topology qualification failure, not a
  dense serving-performance result.
- Fusing the LLMM1 producer and sidecar allreduce into a single kernel was
  exact but slower on `.20/.30`; the simple producer-ready spin-barrier branch
  is closed.
...
| Explicit balanced `RCCL_TREES` | runtime tree string plus specialized RCCL overlay | Replaced RCCL's default one-child chain with an explicit binary tree matched to the TP8 hosts. | Current durable dense decode milestone, not gate clear. |
...
36. Targeted device-7-internal `RCCL_TREES` topology is a rejected scheduling
    ...
    device `7` was internal on active channels. Direct C API timing was exact
    ...
    topology change.
37. The corrected serving wall-clock trace for device-7-internal topology
    closes tree-label retuning more strongly. It improved arrival spread but
    left the run-wall tail on devices `6/7`; internal depth-1 rows became the
